...
let SE_ARMC: Int = 2
let SE_VERTEX: Int = 3
let SE_EQUASC: Int = 4       /* "equatorial ascendant" */
let SE_COASC1: Int = 5       /* "co-ascendant" (W. Koch) */
let SE_COASC2: Int = 6       /* "co-ascendant" (M. Munkasey) */
let SE_POLASC: Int = 7       /* "polar ascendant" (M. Munkasey) */
let SE_NASCMC: Int = 8
...
let SIDT_LTERM_T0: Double = 2396758.5
let SIDT_LTERM_T1: Double = 2469807.5
let SIDT_LTERM_OFS0: Double = (0.000378172 / 15.0)
let SIDT_LTERM_OFS1: Double = (0.001385646 / 15.0)

let VERY_SMALL: Double = 1E-10
...
let O1MAS2DEG: Double = (1 / 3600.0 / 10000000.0)
/* Number of terms in the luni-solar nutation model */
let NLS: Int = 678
let NLS_2000B: Int = 77
...
//                }
        ...
        if (do_earth) {
            if (tjd == swed.pldat[SEI_EMB].teval) {
                ...
                for i in 0..<3 {
                    var tmp: Double = (1 / 0.0123000383)
                    tmp = tmp + 1.0
                    xpe[i] = xpe[i] - xpm[i] / tmp
                }
                /* speed is needed, if
...
//                swi_nutation(tjd, iflag, nutlo)
         ...
         */
        let tmp:Double = swed.topd.geolat * DEG_TO_RAD
        cosfi = cos(tmp);
        sinfi = sin(tmp);
        let tmp2 : Double = 1-f
        let tmp3 : Double = tmp2 * tmp2
        cc = 1 / sqrt(cosfi * cosfi + tmp3 * sinfi * sinfi);
        ss = (1-f) * (1-f) * cc;
...
//                }
                ...
                retc = swi_polcart_sp(tmpReturn3)
                NSLog(retc.serr)
            }
        }
        /************************************************
 ...
 */
    func swi_nutation_iau2000ab(_ J: Double) -> SweRet {
        ...
        tmp = T*(0.051635 + tmp)
        tmp = T*(31.8792 + tmp)
        tmp = T*(1717915923.2178 + tmp)
        tmp = 485868.249036 + tmp
        ...
        tmp = T*( -0.001037 + tmp )
        tmp = T*( -12.7512 + tmp )
        tmp = T*( 1739527262.8478 + tmp )
        tmp = 335779.526232 + tmp
        F   = hlib.swe_degnorm(tmp / 3600.0) * DEG_TO_RAD;
        /* Mean elongation of the Moon from the Sun. */
        tmp = T*( -0.00003169 )
        tmp = T*( 0.006593 + tmp )
        tmp = T*( -6.3706 + tmp )
        tmp = T*( 1602961601.2090 + tmp )
        tmp = 1072260.70369 + tmp
        D   = hlib.swe_degnorm(tmp / 3600.0) * DEG_TO_RAD;
        /* Mean longitude of the ascending node of the Moon. */
        tmp = T*( -0.00005939 )
        tmp = T*( 0.007702 + tmp )
        tmp = T*( 7.4722 + tmp )
        tmp = T*( -6962890.5431 + tmp )
        tmp = 450160.398036 + tmp
        OM  = hlib.swe_degnorm(tmp / 3600.0) * DEG_TO_RAD;
        ...
        for i in (0..<inls).reversed() {
            ...
            tmp += (Double) ((Double)(nls[j + 3]) * D)
            tmp += (Double) ((Double)(nls[j + 4]) * OM)
            darg = hlib.swe_radnorm(tmp);
            sinarg = sin(darg);
            cosarg = cos(darg);
            k = i * 6;
            var tmp2: Double = (Double)(cls[k+0]) + (Double)(cls[k+1]) * T
            dpsi += tmp2 * sinarg + (Double)(cls[k+2]) * cosarg;
            tmp2 = (Double)(cls[k+3]) + (Double)(cls[k+4]) * T
            deps += tmp2 * cosarg + (Double)(cls[k+5]) * sinarg;
        }
    ...
    func swe_houses(tjd_ut: Double, geolat: Double, geolon: Double, hsys: String) -> SweRet {
        var ret: SweRet = SweRet()
    ...
    func swe_houses_armc(_ armc: Double, geolat: Double, eps: Double, hsys: String, ascmc9: Int) -> SweRet
    {
        ...
        let armc1: Double = hlib.swe_degnorm(armc);
        if (hsys ==  "I" || hsys == "i") {  // declination for sunshine houses
            if (ascmc9 == 99) {
                h.sundec = 0;
                if (saved_sundec != 99) {
                    h.sundec = saved_sundec;
                }
            } else {
                h.sundec = (Double)(ascmc9)
                saved_sundec = h.sundec;
            }
        }
        retc = hcalc.CalcH(armc1, fi: geolat, ekl: eps, hsy: hsys, iteration_count: 2);
        retc.cusps[0] = 0;
        for i in 1..<ito+1 {
